...
def main(sourceFolder, docsDir: str, ghToken: str, priorityOnlyMode: bool) -> None:
	...
	if not priorityOnlyMode:
		# Make tdx
		with open(path.join(docsDir, "unistore", "db-nds-shop.tdx"), "wb") as tdx:
		...
		with open(path.join(tempDir, "48", "icons.t3s"), "w", encoding="utf8") as file:
			...
			for i in range(iconIndex):
				file.write(f"{i}.png\n")
		system(f"tex3ds -i {path.join(tempDir, '48', 'icons.t3s')} -o {path.join(docsDir, 'unistore', 'db-nds-shop.t3x')}")

	# Write UniStore and metadata
	unistore.save(path.join(docsDir, "unistore", "db-nds-shop.unistore"), path.join(docsDir, "unistore", "db-nds-shop.json"))
